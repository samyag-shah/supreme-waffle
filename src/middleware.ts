...
const authRoutes = ['/user/signin','/user/signup', '/owner/signup','/owner/signin']
...
export function middleware(request: NextRequest) {
    ...
    if (currentUser) {
        ...
        if(currentUser1.userType === "user"){
            ...
            } else if (ownerRoutes.includes(request.nextUrl.pathname) && currentUser) {
                return NextResponse.redirect(new URL('/', request.url))
            }
        } else if (currentUser1?.userType === "owner") {
            if(authRoutes.includes(request.nextUrl.pathname) && currentUser) {
                return NextResponse.redirect(new URL('/owner', request.url))
            } else if (userRoutes.includes(request.nextUrl.pathname) && currentUser) {
                return NextResponse.redirect(new URL('/owner', request.url))
            }
        }
        }
